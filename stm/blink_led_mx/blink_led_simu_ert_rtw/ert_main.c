...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) blink_led_simu_M->Timing.taskTime0;
  blink_led_simu_step();
...
int main(int argc, char **argv)
{
  ...
  blink_led_simu_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(blink_led_simu_M->extModeInfo, &rtmGetTFinal
    (blink_led_simu_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(blink_led_simu_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(blink_led_simu_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
